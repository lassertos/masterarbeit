...
import { FileServiceEvent } from "./messages.js";
...
export class FileService__Producer implements Service {
  ...
  constructor(serviceId: string) {
    console.log("creating file service producer", serviceId);
  ...
  async sendFile(fileType: string, content: Uint8Array) {
    if (this.channel !== undefined) {
      ...
      this.channel.send(
        JSON.stringify({ fileType: fileType, length: content.length })
      );
  ...
  handleData(data: string | Blob | ArrayBuffer | ArrayBufferView) {
    if (typeof data === "string") {
      ...
      if (this.dataRead === this.header.length) {
        this.emit("file", {
          file_type: this.header.fileType,
