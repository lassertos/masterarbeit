...
import { SignalingMessage } from "../deviceMessages.mjs";
import { logger } from "../logging.mjs";
import { assert } from "../utils.mjs";
import { Channel, MediaChannel } from "./channel.mjs";
import {
  PeerConnection,
  ...
  implements PeerConnection
{
  ...
  private onnegotiationneeded() {
    if (this._state !== ConnectionState.Calling) {
      logger.log(
        "error",
        "onnegotiationneeded fired but state is not Calling",
        {
          state: this._state,
        }
      );
    ...
    this.pc.ondatachannel = (event) => {
      ...
      if (channel !== undefined && channel.channel_type === "DataChannel") {
        ...
        try {
          ...
          logger.log("error", "can not set channel ready", {
            error: e as Error,
          });
        }
      }
  ...
  private async makeOffer() {
    ...
    logger.log("trace", "WebRTCPeerConnection.makeOffer created offer", {
      offer,
    });
    ...
    if (!_offer) {
      logger.log(
        "info",
        "WebRTCPeerConnection.makeOffer failed to create offer"
      );
    ...
    logger.log("trace", "WebRTCPeerConnection.makeOffer updated offer", {
      offer,
    });
    this.emit("signalingMessage", {
      signalingType: "offer",
  ...
  private async makeAnswer(offer: RTCSessionDescriptionInit) {
    ...
    logger.log("trace", "WebRTCPeerConnection.makeAnswer created answer", {
      answer,
    });
    ...
    answer = _answer;
    logger.log("trace", "WebRTCPeerConnection.makeAnswer updated answer", {
      answer,
    });
    if (answer.sdp === undefined) {
      throw new Error(
        "WebRTCPeerConnection.makeAnswer answer.sdp is undefined"
      );
    }
  ...
  private modifySDP(sdpString: string): string {
    ...
    for (const transeiver of this.transeiverMap.keys()) {
      ...
      logger.log(
        "trace",
        "WebRTCPeerConnection.modifySDP setTrack id for transeiver",
        {
          transeiver,
          mid: transeiver.mid,
          label,
      ...
      if (sectionIdx !== -1) {
        const modifiedSection = sections[sectionIdx].replace(
          msidRegex,
          "$1" + label
        );
    ...
    logger.log("trace", "WebRTCPeerConnection.modifySDP returns", {
      sdpString,
    });
    return sdpString;
  }
  ...
  private matchMediaChannels() {
    ...
    for (const transceiver of transceivers) {
      ...
      logger.log(
        "trace",
        "WebRTCPeerConnection.matchMediaChannels matching tramceiver",
        {
          transceiver,
          label,
        }
      ...
      if (channel.track) {
        ...
        logger.log(
          "trace",
          "WebRTCPeerConnection.matchMediaChannels replace track"
        );
