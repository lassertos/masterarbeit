...
import { DataMessage, FileEventMessage, Message } from "./types";
import fs from "fs";
import path from "path";

class ClangdLanguageServerProvider {
    private clients: Map<
        string,
        {
            webSocket: WebSocket;
    ...
    private registerClient(webSocket: WebSocket) {
        ...
        clangdProcess.on("close", (code) => {
            console.log(
                `clangd process of client ${uuid} exited with code ${code}`
            );
        ...
        webSocket.on("message", (data) => {
            ...
            if (message.type === "data")
                return this.receiveMessage(uuid, message);
            if (message.type === "file-event")
                return this.handleFileEvent(message);
        });
    }
...
new ClangdLanguageServerProvider({ webSocketServer: { port: 3010 } });
